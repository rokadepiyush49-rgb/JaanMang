...
import { toViewAchievements } from "@/lib/student/adapters";
...
export function ProfilePage() {
  /* All of it from the student workspace now. The impact record used to be a
     fixture sitting under a real name, which is the most misleading
     combination this screen could have had. */
  const { state } = useStudent();
  const profile = state.profile!;
  const badges = state.achievements ? toViewAchievements(state.achievements) : [];
  const earned = badges;
  const completed = state.projects.filter((p) => p.phase === "completed").length;

  const name = profile.name;
  const institution = profile.institution.name;
  const location = `${profile.district}, ${profile.state}`;
  const year = `Year ${profile.currentYear} · ${profile.branch}`;
  const role = studentRole(profile);
  const completeness = profileCompleteness(profile);
  const skills = profile.skills;
  const interests = profile.interests;
  /* The impact ledger: every row is a verified contribution the platform
     recorded, not a story typed into a fixture. */
  const contributions = (state.achievements?.ledger ?? []).map((entry) => ({
    icon: "star" as const,
    tone: "navy" as const,
    title: entry.source,
    date: new Date(entry.at).toLocaleDateString("en-IN", { dateStyle: "medium" }),
    body: "",
    kind: "Verified contribution",
    points: `+${entry.points} Impact Pts`,
  }));
      ...
      <Enter index={2}>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:gap-4">
          <TintTile
            ...
            label="Impact points"
            tint="navy"
            value={(state.achievements?.totalPoints ?? 0).toLocaleString("en-IN")}
          />
          <TintTile
            icon="check-circle"
          ...
          <TintTile
            ...
            label="State rank"
            tint="amber"
            value={state.achievements?.rank ? `#${state.achievements.rank.rank}` : "—"}
          />
        </div>
      </Enter>
