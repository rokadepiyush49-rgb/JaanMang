...
import { toViewAchievements, type ViewAchievement } from "@/lib/student/adapters";
 ...
 */
function BadgeTile({ badge }: { badge: ViewAchievement }) {
...
export function AchievementsPage() {
  ...
  const badges = achievements ? toViewAchievements(achievements) : [];
  /* The campus leaderboard is the recognition layer’s students scope. It is
     empty until the cron in stage 07 materialises it, and an empty ranking is
     better than a fabricated one. */
  const leaderboard: { rank: number; name: string; points: number }[] = [];
  const earned = badges;
    /* No locked list. A badge is earned from a rule the recognition cron
     evaluates; until it has run there is no partial state, and a progress bar
     toward an unearned badge is a number nothing produces. */
  const locked: ViewAchievement[] = [];
      ...
      <Enter index={1}>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:gap-4">
          ...
          <TintTile
            ...
            label="Impact score"
            tint="navy"
            value={(achievements?.totalPoints ?? 0).toLocaleString("en-IN")}
          />
          <TintTile
            icon="trending-up"
            label="State rank"
            tint="mint"
            value={(achievements?.rank ? `#${achievements.rank.rank}` : "—")}
          />
          <TintTile
            icon="flame"
            label="Day streak"
            tint="clay"
            value={String(achievements?.verifiedContributions ?? 0)}
          />
        </div>
      </Enter>
